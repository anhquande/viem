...
public class AppSettingManager extends CacheManager<AppSetting>{
	public static final Logger log = Logger.getLogger(AppSettingManager.class.getName());

	@Inject
	protected AppSettingManager(AppSettingDao dao){
		log.info("Init AppSettingManager ...");
		createDefaultSettings();
		this.dao = dao;
	}
	
	public AppSetting findByKey(final String name){
		if ((name==null) || (name.isEmpty()))
			return null;
		
		for (AppSetting setting : cache) {
			
			String curName = setting.getName();
	...
	public String getSettingValue(final String name){
		AppSetting setting = findByKey(name);
		if (setting!=null)
	...
	public List<AppSetting> getAll(){
		log.info("Get all settings from cache ... ");
	...
	public void createDefaultSettings() {
		...
		addDefaultSetting(7, AppSetting.GROUP_APPLICATION, AppSetting.APP_SIDEBAR, "Sidebar", "/category;Speisekarte|/category/vorspeisen;Vorspeisen|/category/huehnerfleisch;Hühnerfleisch|/category/huehnerbrust;Panierter Hühnerbrust|/category/knuspriges-haehnchen;Knuspriges Hähnchen|/category/knusprige-ente;Knusprige Ente|/category/rindfleisch;Rindfleisch|/category/fisch;Fisch|/category/hummerkrabben;Hummerkrabben|/category/spezialitaeten;Spezialitäten|/category/vegetarisch;Vegetarisch|/category/kleine-portionen;Kleine Portionen|/category/beilagen;Beilagen|/category/desserts;Desserts");
		addDefaultSetting(8, AppSetting.GROUP_APPLICATION, AppSetting.APP_PAGE_META, "Page Metadata", "<meta name=\"geo.placename\" content=\"Burtscheider Straße 27, 52064 Aachen, Germany\" />");
		addDefaultSetting(9, AppSetting.GROUP_APPLICATION, AppSetting.APP_LOGO, "Logo", "/themes/thaiexpress/icon/logo.png");
